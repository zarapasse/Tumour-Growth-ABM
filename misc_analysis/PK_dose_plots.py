...
def figure_clearance_vs_accumulation(
    ...
    fig, axes = plt.subplots(2, 1, figsize=(5.2, 4.8), sharex=True, constrained_layout=True)
    ...
    ax.set_title(rf"$\mathbf{{({title_left_labels[0]})}}$ Drug Concentration ($\alpha$={alpha_low})", loc="left", fontsize=10)
    ...
    ax.set_ylim(0, ymax)
    ax.grid(True, alpha=0.3)
    ax.set_title(rf"$\mathbf{{({title_left_labels[1]})}}$ Drug Concentration ($\alpha$={alpha_high})", loc="left", fontsize=10)
...
def figure_even_vs_uneven_side_by_side_alphas(
    ...
    fig, axes = plt.subplots(2, 2, figsize=(10.4, 4.8), sharex=True, sharey=True, constrained_layout=True)

    # ---- Top row: EVEN ----
    ax = axes[0, 0]
    ax.plot(t, c_e_low, lw=2, label="Even", color="black")
    ax.fill_between(t, 0, c_e_low, alpha=0.25, color="black")
    for td in dts: ax.axvline(td, ls="--", lw=1, alpha=0.5, color="black")
    ax.grid(True, alpha=0.3)
    ax.set_title(rf"$\mathbf{{({left_labels[0][0]})}}$ Even ($\alpha$={alpha_low})", loc="left", fontsize=10)
    ax.set_ylabel("Conc. (mg/L)")
    ax.set_ylim(0, ymax)
    ...
    for td in dts: ax.axvline(td, ls="--", lw=1, alpha=0.5, color="black")
    ax.grid(True, alpha=0.3)
    ax.set_title(rf"$\mathbf{{({left_labels[0][1]})}}$ Even ($\alpha$={alpha_high})", loc="left", fontsize=10)
    ax.set_ylim(0, ymax)
    ax.legend(loc="upper right", fontsize=8, frameon=True)
    ...
    for td in dts: ax.axvline(td, ls="--", lw=1, alpha=0.5, color="black")
    ax.grid(True, alpha=0.3)
    ax.set_title(rf"$\mathbf{{({left_labels[1][0]})}}$ Uneven ($\alpha$={alpha_low})", loc="left", fontsize=10)
    ax.set_xlabel("Time (days)")
    ax.set_ylabel("Conc. (mg/L)")
    ax.set_ylim(0, ymax)
    ax.legend(loc="upper right", fontsize=8, frameon=True)
    ...
    for td in dts: ax.axvline(td, ls="--", lw=1, alpha=0.5, color="black")
    ax.grid(True, alpha=0.3)
    ax.set_title(rf"$\mathbf{{({left_labels[1][1]})}}$ Uneven ($\alpha$={alpha_high})", loc="left", fontsize=10)
    ax.set_xlabel("Time (days)")
    ax.set_ylim(0, ymax)
    ax.legend(loc="upper right", fontsize=8, frameon=True)
...
figure_clearance_vs_accumulation(schedule, alpha_low=1, alpha_high=4, t_end=2.0, savepath="misc_analysis/graphs/PK_Clearance_vs_Accumulation.png")
...
figure_even_vs_uneven_side_by_side_alphas(
    even_schedule, uneven_schedule,
    alpha_low=1, alpha_high=4,
    t_end=2.0, savepath="misc_analysis/graphs/PK_Even_vs_Uneven_Side_by_Side.png"
)