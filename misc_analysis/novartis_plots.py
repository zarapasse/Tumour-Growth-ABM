...
df_ctrl = df[(df["Treatment"] == TREATMENT) & (df["TumourType"] == TUMOUR_TYPE)].copy()
...
plt.figure(figsize=(6.5, 4.5))
plt.scatter(df_ctrl["TimeToDouble"], df_ctrl["BestResponse"], alpha=0.75)
...
plt.tight_layout()
plt.savefig(
    "misc_analysis/graphs/Novartis_TimeToDouble_vs_BestResponse_Scatter.png", dpi=300
)
plt.show()
...
df_untreated.boxplot(column="TimeToDouble", by="TumourType", grid=False)
