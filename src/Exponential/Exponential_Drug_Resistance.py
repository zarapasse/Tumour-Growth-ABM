...
class ResistantTumourCell(TumourCell):
    ...
    def __init__(self, model):
        ...
        self.p_birth = model.p_birth
        self.p_death = 1 - np.exp(-model.death_rate * model.dt)  # baseline death only
...
class TumourModel(Model):
    ...
    def __init__(
        ...
        self.datacollector = DataCollector(
            model_reporters={
                "Sensitive": lambda m: sum(1 for a in m.agents if isinstance(a, TumourCell) and not isinstance(a, ResistantTumourCell)),
                "Resistant": lambda m: sum(1 for a in m.agents if isinstance(a, ResistantTumourCell)),
