    ...
    def step(self):
        ...
        if self.model.rng.random() < p_event:
            if self.model.rng.random() < (b / r):
                if self.model.enable_resistance and (
                    self.model.rng.random() < self.model.p_mutation
                ):
...
class ResistantTumourCell(Agent):
    ...
    def step(self):
        ...
        d = (
            self.model.death_rate
        )  # resistant cells are unaffected by drug, so use base death rate
        r = b + d
